...
%{\color{blue}• First, you need to have a thorough knowledge about everything that has been
...
%	
	%\section{Basic deterministic model with \textit{WASH}}\label{sec:model1}
...
%		 (suponiendo un sistema de apareamiento poligámico). A modo de ejemplo si se supone un modelo binomial negativo para la distribución de parásitos, las expresiones para $\psi$ y $\phi$ vienen dadas por \cite{anderson1992infectious}
	...
	%mantener el control. 
	Las buenas prácticas de higiene, como el lavado de manos y aseo personal son medidas que previenen la infección. Además, en los lugares de riesgo, el uso de calzado es importante para que los individuos no se infecten por la tierra contaminada. Por otro lado %mejora y 
	...
	% ya que 1 gramo de heces de un individuo infectado puede contener hasta 100 huevos de parásitos (ref).
	  Por lo tanto
	...
	\begin{itemize}
		...
		\item ``\textit{saneamiento}" : esta intervención reducen la contribución de las personas al reservorio (por ejemplo, %uso de letrinas
...
%	}
	...
	%a la población de hospedadores (o parte de la población) se le implementan las intervención de higiene.
	%Supondremos que esta se realiza sobre un porcentaje $p$ de la población. 
%	Esta acción divide a la población en dos grupos, los individuos que tienen acceso a estas y los que no (por ejemplo los individuos que no tienen acceso a agua potable segura), cada grupo con un porcentaje de población $p_1=1-p$ y $p_2=p$  respectivamente. Su dinámica esta descripta en el sistema (\ref{model1hetero}).
	También suponemos que las intervenciones reducen la tasa de contacto de los individuos con el reservorio, pero sin embargo no reducen la tasa de contribución% de los individuos con el reservorio
	, por lo tanto el valor de los parámetros $\rho_1$ y $\rho_2$ es el mismo, digamos $\rho$. 
	%También supondremos que esta implementación no reduce la contribución de los individuos con el reservorio, por lo tanto el valor de los parámetros $\rho_1$ y $\rho_2$ es el mismo y sera simplemente $\rho$. 
	%Por lo ultimo expuesto y sabiendo que los individuos comparten un mismo reservorio (ambiente contaminado) obtenemos que los $M_i$ son todos iguales, y los denotaremos simplemente por $M$. 
	%Por otro lado el uso del agua potable reduce la tasa de contacto de los individuos con el reservorio, entonces los valores de $\beta_1$ y $\beta_2$ son distintos donde $\beta_1=\beta$ 	
